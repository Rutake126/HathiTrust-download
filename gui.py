...
class HathiTrustApp(ctk.CTk):
    def __init__(self):
        ...
        self._parsed_book_id = None  # 解析成功后才设置，下载前必须有值
    ...
    def _find_browser(self) -> str:
        """返回 Chrome/Edge 路径；Chrome 优先，Edge 兜底。"""
        candidates = (
    ...
    def _on_url_changed(self, *_args):
        """URL 输入框内容变化时，清除已解析状态，强制用户重新点击解析"""
        self._parsed_book_id = None
    ...
    def _parse_url(self):
        ...
        if config and config.get("book_id"):
            self._parsed_book_id = config["book_id"]
    ...
    def _validate_inputs(self) -> dict | None:
        # 必须先点击「解析」按钮成功提取 Book ID
        if not self._parsed_book_id:
            messagebox.showerror("错误", "请先点击「解析」按钮提取 Book ID")
            return None
    ...
    def _download_image_via_browser(self, page, url: str, filepath: str,
                                    timeout: float = 60.0):
        """
        通过浏览器会话读取图片字节，再由 Python 写入文件。

        这不是触发 Chrome 下载，不受下载栏/自动下载策略影响。
        返回 (success, message, size_bytes)。
        """
        import base64
        ...
        try:
            result = page.run_js(js, url, timeout=timeout)
            if not isinstance(result, dict) or not result.get("ok"):
                return False, (result or {}).get("msg", "浏览器未返回图片数据"), 0

            data = base64.b64decode(result.get("data") or "", validate=True)
            if not data:
                return False, "浏览器返回空图片", 0
            if data[:3] != b'\xff\xd8\xff':
                return False, f"非 JPEG 数据 ({len(data)} 字节, {result.get('type')})", 0
    ...
    def _download_worker(self, params: dict):
        ...
        try:
            ...
            # 显示 URL 模板
            template_url = build_download_url(book_id, seq=1, size=size, attachment=0)
            ...
            # 检查路径是否被同名文件占用
            if os.path.exists(book_folder) and not os.path.isdir(book_folder):
                self._log(f"❌ 路径已被同名文件占用: {book_folder}")
                self._log("   请删除该文件或更换保存目录")
                self._update_progress_label("路径冲突")
                return

            try:
                os.makedirs(book_folder, exist_ok=True)
            except PermissionError:
                self._log(f"❌ 无权限创建目录: {book_folder}")
                self._log("   请检查目录是否可写，或更换保存目录")
                self._update_progress_label("权限不足")
                return
            except OSError as e:
                self._log(f"❌ 创建目录失败: {e}")
                self._update_progress_label("目录创建失败")
                return
            ...
            self._update_progress_label("准备下载...")

            self._update_progress_label("启动浏览器...")
            ...
            def ensure_browser_page():
                ...
                self._log(f"🌐 启动浏览器: {browser_path}")

                # 使用独立的用户数据目录和端口，避免与用户正在使用的浏览器冲突
                import tempfile
                user_data_dir = os.path.join(tempfile.gettempdir(), "HathiTrust_browser_profile")
                os.makedirs(user_data_dir, exist_ok=True)

                options = ChromiumOptions()
                options.set_browser_path(browser_path)
                options.set_argument(f'--user-data-dir={user_data_dir}')
                options.set_local_port(19222)  # 固定调试端口，避免与默认 Chrome 冲突
                if proxy:
                ...
                # 访问 HathiTrust 首页建立同源上下文和 Cookie
                self._log("  ⏳ 正在初始化浏览器会话...")
                try:
                ...
                # 再访问一张小图预热 CDN / 图片接口
                warmup_url = build_download_url(
                    book_id, seq=start_seq, size="200",
                    fmt="image/jpeg", attachment=0,
                )
                try:
                    page.get(warmup_url)
            ...
            for i, seq in enumerate(range(start_seq, end_seq + 1)):
                ...
                url = build_download_url(book_id, seq, size=size, attachment=0)
                ...
                for attempt in range(retry_count + 1):
                    ...
                    try:
                        page = ensure_browser_page()
                        browser_ok, browser_msg, browser_size = self._download_image_via_browser(
                            page=page,
                            url=url,
                            filepath=filepath,
                            timeout=max(45.0, retry_interval + 30.0),
                        )
                    except Exception as ex:
                        last_error = f"上下文丢失: {ex}"
                        self._log(f"  ↻ 页面上下文丢失，恢复中...")
                        # 关闭死掉的浏览器，下次 ensure_browser_page 会重建
                        if page is not None:
                            try:
                                page.quit()
                            except Exception:
                    ...
                    if browser_ok:
                        if attempt == 0:
                            self._log(f"  ✓ {filename} ({browser_size // 1024} KB)")
                        else:
                            self._log(f"  ✓ {filename} ({browser_size // 1024} KB, 第 {attempt + 1} 次成功)")
                        downloaded += 1
                        success = True
                        break
                    else:
                        last_error = browser_msg
                        if attempt < retry_count:
