...
def find_browser() -> str:
    """
    在 Windows 上自动检测 Chrome / Edge 浏览器可执行文件路径。
    按优先级查找：Chrome 标准路径 → Chrome 用户路径 → Edge 标准路径 → 注册表
    找不到则返回空字符串。
    """
    candidates = [
...
if not _browser_path:
    ...
    print("   或 Edge (https://www.microsoft.com/edge)")
    exit(1)
print(f"🌐 浏览器: {_browser_path}")
...
# ============ 预热：提前触发 DNS / TLS / Cookie / CDN 缓存 ============
print("\n🔥 预热浏览器与网络连接...")
_warmup_url = build_download_url(BOOK_ID, seq=START_SEQ, size="200",
                                 fmt="image/jpeg", attachment=0)
_warmup_js = f"""
(async () => {{
    try {{
        const r = await fetch({_warmup_url!r}, {{credentials: 'include'}});
        if (!r.ok) return 'bad:' + r.status;
        const blob = await r.blob();
        return 'ok:' + r.status + ':' + blob.size;
    }} catch (e) {{
...
for _attempt in range(3):
    try:
        page.run_js(_warmup_js)
    except Exception as _ex:
        print(f"  ↻ 预热第 {_attempt + 1} 次失败: {_ex}")
        time.sleep(2 * (_attempt + 1))
        continue
    time.sleep(2.5)
    _warmup_ok = True
    print(f"  ✓ 预热完成 (第 {_attempt + 1} 次)")
    break
if not _warmup_ok:
    print("⚠ 预热未成功，仍尝试进入下载（首页可能需要更多重试）")
